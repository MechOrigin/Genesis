...
public static final String name = "Mind";
...
public Mind() throws IOException {
...
public boolean respond(String s) { //decide how and what to respond, return if we should keep the program alive
    ...
    if(last == null) {
        ...
        int i = 1;
        int numberofiterations = 1; //how many times to run querry
        
//        StringBuilder sentence = new StringBuilder(); //stringbuilder stuff for more powerful string functions
        
        while (i <= numberofiterations) {
        	Wikiparser.questiontype = s;
        	try {
    			Wikiparser.main();
    		} catch (IOException e) {
        	...
        	for(String r : fm.getResponses(ResponseType.QUESTION)) {
        		if(transform(s).equalsIgnoreCase(transform(r)))
        			isNewQuestion = false; //dont store it again
        	}
        	if(isNewQuestion && s.contains("?") && s.contains("who")) //store a new greeting for use in another session (or this one)
                fm.setResponse(ResponseType.WHO_RESPONSE, s);
            //give a greeting back!
        	if(isNewQuestion && s.contains("?") && s.contains("who") ) {
        	 System.out.println(response = (name + ": " + format(fm.getResponses(ResponseType.MOST_RELEVANT_DATA).get((int) (System.nanoTime() % fm.getResponses(ResponseType.MOST_RELEVANT_DATA).size())))));
        	}
        	
        	if(isNewQuestion && s.contains("?") && s.contains("what"))
        		fm.setResponse(ResponseType.WHAT_RESPONSE, s);
        	if(isNewQuestion && s.contains("?") && s.contains("what") ) {
        	System.out.println(response = (name + ": " + format(fm.getResponses(ResponseType.MOST_RELEVANT_DATA).get((int) (System.nanoTime() % fm.getResponses(ResponseType.MOST_RELEVANT_DATA).size())))));
        	}
 /*       	
        	//outputs the most relevant WHO data
        	if(isNewQuestion && s.contains("?") && s.contains("who") ) {
        	String whoResponse = String.join(" ", fm.getResponses(ResponseType.MOST_RELEVANT_DATA));
        	System.out.println(response = (name + ": ") + whoResponse);
        	}
        	
        	//outputs the most relevant WHAT data
        	if(isNewQuestion && s.contains("?") && s.contains("what") ) {
        	String whatResponse = String.join(" ", fm.getResponses(ResponseType.MOST_RELEVANT_DATA));
        	System.out.println(response = (name + ": ") + whatResponse);
        	}
*/
        	i++;
        }
        // todo: if responses QUERRY options are greater than 8 then search all of those too
        
        
        //todo: add a way to filter out specific words and then spit them to (s) string and do another querry search
//        String output = String.join(" ", fm.getResponses(ResponseType.QUERRY2));
//        System.out.println(output);
 //   	if(s.equalsIgnoreCase(fm.getResponses(ResponseType.QUERRY2).toString())) {
 //   	}
        
        
        // THIS CHUNK IS TO LEARN SPECIFIC WORDS AND SAVE THEM //
        //take the question asked and split it up into words
        String stri=s;
...
//            System.out.println(output2);
        ...
        // This is a function that takes all learned words and throws them, excluding whatever is in output2 //
/*        
        List<String> li=fm.getResponses(ResponseType.LEARNED_WORDS);
...
//        }
        ...
        for(String st:li){
//            if(st.equalsIgnoreCase("james") | st.equalsIgnoreCase("lebron")){
        	if(st.equalsIgnoreCase(output2)) {
                // Do nothing
            } else {
                finalLi.add(st);
            }
        ...
        CheckFile.findwords = output2;
        CheckFile.findword();
        System.out.println("2" + " " + CheckFile.findwords); 
        System.out.println("3" + " " + fm.getResponses(ResponseType.MOST_RELEVANT_DATA));
//        	fm.setResponse(ResponseType.VALUE, removeEndPunc(s));
//        	System.out.println(response = (name + ": " + format(fm.getResponses(ResponseType.QUESTION).get((int) (System.nanoTime() % fm.getResponses(ResponseType.QUESTION).size())))));

//        System.out.println(format(fm.getResponses(ResponseType.QUESTION).get((int) (System.nanoTime() % fm.getResponses(ResponseType.QUESTION).size())))); //this shows the last question
*/        
    }
    
    //put a condition here to continue the querry
